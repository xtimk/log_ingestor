...
namespace FSWriter.Services.MessageService.Impl
{
    public class KafkaConsumer<T> : IMessageConsumer<T>
    {
        ...
        public KafkaConsumer(ILogger<KafkaConsumer<T>> logger, IJsonSerializer<T> jsonSerializer, IMetricsService metricsService)
        {
            ...
            _baseLogMessage = $"Kafka Consumer[{_service_guid}]: ";
        }
        ...
        public void Subscribe(string topic)
        {
            ...
            using (var adminClient = new AdminClientBuilder(new AdminClientConfig { BootstrapServers = _config.BootstrapServers }).Build())
            {
                ...
                {
                    _logger.LogError($"{_baseLogMessage}An error occured creating topic {e.Results[0].Topic}: {e.Results[0].Error.Reason}");
                }
            }
            ...
            while(!_cancelled)
            {
                ...
                {
                    throw new Exception($"{_baseLogMessage}Deserialized message is null");
                }
